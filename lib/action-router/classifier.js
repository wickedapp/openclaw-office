...
export function classifyTask(input = {}) {
  ...
  if (hasAny(t, [
    /telegram.*(send|message|post|notify|notification)/,
    /(send|message|post|notify|notification).*telegram/,
  ])) {
    reasons.push('matched dangerous Telegram notification keyword')
    return { taskType: TASK_TYPES.TELEGRAM_NOTIFICATION, confidence: 'high', reasons, text }
  }
